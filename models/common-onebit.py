...
def l1_norm(layer):
    return torch.max(torch.sum(torch.abs(layer.weight.data), dim=0))

def apply_bit_layer(input, layer, layer_norm, quant_activation):

    # "Quant(x)"
    input = layer_norm(input)
    input = quant_activation(input)

    beta = l1_norm(layer)
    gamma = torch.max(torch.abs(input))
    return layer(input) * beta * gamma / quant_activation.max_val
...
class QuantActivation(nn.Module):
    ...
    def forward(self, input):
        gamma = torch.max(torch.abs(input))
        return clip(input * self.max_val / gamma, -self.max_val + self.eps, self.max_val - self.eps)

class BitLinear(nn.Module):
    def __init__(self, c1, c2, bias=True):
        super(BitLinear, self).__init__()
        self.linear = BitWeights(nn.Linear(c1, c2, bias=bias))
        self.ln = nn.LayerNorm(c1)
        self.qact = QuantActivation()

    def forward(self, input):
        return apply_bit_layer(input, self.linear, self.ln, self.qact)
...
class BitConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        super().__init__()
        self.conv = BitWeights(nn.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False))
        self.bn = nn.BatchNorm2d(c2)
        self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
        self.ln = nn.GroupNorm(1, c1)
        self.qact = QuantActivation()

    def forward(self, x):
        """Applies a convolution followed by batch normalization and an activation function to the input tensor `x`."""
        return self.act(self.bn(apply_bit_layer(x, self.conv, self.ln, self.qact)))

    def forward_fuse(self, x):
        """Applies a fused convolution and activation function to the input tensor `x`."""
        return self.act(apply_bit_layer(x, self.conv, self.ln, self.qact))



class BitDWConv(BitConv):
...
class BitConvTranspose2d(nn.ConvTranspose2d):
    def __init__(self, c1, c2, k=1, s=1, p1=0, p2=0):
        """Initializes a depth-wise transpose convolutional layer for YOLOv5; args: input channels (c1), output channels
        (c2), kernel size (k), stride (s), input padding (p1), output padding (p2).
        """
        super().__init__(c1, c2, k, s, p1, p2, groups=math.gcd(c1, c2))
        self.conv = BitWeights(nn.ConvTranspose2d(c1, c2, k, s, p1, p2, groups=math.gcd(c1, c2)))
        self.ln = nn.GroupNorm(1, c1)
        self.qact = QuantActivation()

    def forward(self, x):
        return apply_bit_layer(x, self.conv, self.ln, self.qact)


# "sublayer norm": https://arxiv.org/pdf/2210.06423
class BitTransformerLayer(nn.Module):
    ...
    def __init__(self, c, num_heads):
        ...
        """
        super().__init__()
        self.q = BitLinear(c, c, bias=False)
        self.k = BitLinear(c, c, bias=False)
        self.v = BitLinear(c, c, bias=False)
        self.ma = nn.MultiheadAttention(embed_dim=c, num_heads=num_heads)
        self.fc1 = BitLinear(c, c, bias=False)
        self.fc2 = BitLinear(c, c, bias=False)

        self.subln = nn.LayerNorm(c)

    def forward(self, x):
        """Performs forward pass using MultiheadAttention and two linear transformations with residual connections."""
        
        subln_x = self.subln(x)
        x = self.ma(self.q(subln_x), self.k(subln_x), self.v(subln_x))[0]
        x = self.fc2(self.fc1(self.subln(x))) + x
        return x
...
class BitTransformerBlock(nn.Module):
    ...
    def __init__(self, c1, c2, num_heads, num_layers):
        ...
        self.conv = None
        if c1 != c2:
            self.conv = BitConv(c1, c2)
        self.linear = BitLinear(c2, c2)  # learnable position embedding
        self.tr = nn.Sequential(*(BitTransformerLayer(c2, num_heads) for _ in range(num_layers)))
        self.c2 = c2
...
class BitBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):
        ...
        self.cv1 = BitConv(c1, c_, 1, 1)
        self.cv2 = BitConv(c_, c2, 3, 1, g=g)
...
class BitBottleneckCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = BitConv(c1, c_, 1, 1)
        self.cv2 = BitWeights(nn.Conv2d(c1, c_, 1, 1, bias=False))
        self.cv3 = BitWeights(nn.Conv2d(c_, c_, 1, 1, bias=False))
        self.cv4 = BitConv(2 * c_, c2, 1, 1)
        self.bn = BitWeights(nn.BatchNorm2d(2 * c_))  # applied to cat(cv2, cv3)
        self.act = nn.SiLU()
        self.m = nn.Sequential(*(BitBottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))

        self.ln2 = nn.GroupNorm(1, c1)
        self.ln3 = nn.GroupNorm(1, c_)
        self.qact = QuantActivation()
    ...
    def forward(self, x):
        ...
        enhanced output.
        """
        y1 = apply_bit_layer(self.m(self.cv1(x)), self.cv3, self.ln2, self.qact)
        y2 = apply_bit_layer(x, self.cv2, self.ln2, self.qact)
        return self.cv4(self.act(self.bn(torch.cat((y1, y2), 1))))

class BitCrossConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1, g=1, e=1.0, shortcut=False):
        ...
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = BitConv(c1, c_, (1, k), (1, s))
        self.cv2 = BitConv(c_, c2, (k, 1), (s, 1), g=g)
        self.add = shortcut and c1 == c2
...
class BitC3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = BitConv(c1, c_, 1, 1)
        self.cv2 = BitConv(c1, c_, 1, 1)
        self.cv3 = BitConv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(BitBottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
...
class BitC3x(BitC3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        c_ = int(c2 * e)
        self.m = nn.Sequential(*(BitCrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)))
...
class BitC3TR(BitC3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = BitTransformerBlock(c_, c_, 4, n)


class BitC3SPP(BitC3):
    # C3 module with SPP()
    def __init__(self, c1, c2, k=(5, 9, 13), n=1, shortcut=True, g=1, e=0.5):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = BitSPP(c_, c_, k)


class BitC3Ghost(BitC3):
    # C3 module with GhostBottleneck()
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        """Initializes YOLOv5's C3 module with Ghost Bottlenecks for efficient feature extraction."""
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(BitGhostBottleneck(c_, c_) for _ in range(n)))


class BitSPP(nn.Module):
    ...
    def __init__(self, c1, c2, k=(5, 9, 13)):
        ...
        self.cv1 = BitConv(c1, c_, 1, 1)
        self.cv2 = BitConv(c_ * (len(k) + 1), c2, 1, 1)
...
class BitFocus(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        ...
        """
        super().__init__()
        self.conv = BitConv(c1 * 4, c2, k, s, p, g, act=act)
...
class BitGhostConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, g=1, act=True):
        ...
        self.cv1 = BitConv(c1, c_, k, s, None, g, act=act)
        self.cv2 = BitConv(c_, c_, 5, 1, None, c_, act=act)
...
class BitGhostBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1):
        ...
        self.conv = nn.Sequential(
            BitGhostConv(c1, c_, 1, 1),  # pw
            BitDWConv(c_, c_, k, s, act=False) if s == 2 else nn.Identity(),  # dw
            BitGhostConv(c_, c2, 1, 1, act=False),
        )  # pw-linear
        self.shortcut = (
            nn.Sequential(BitDWConv(c1, c1, k, s, act=False), BitConv(c1, c2, 1, 1, act=False)) if s == 2 else nn.Identity()
        )
